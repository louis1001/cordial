...
private:
    ...
    std::optional<Token> token_para_palabra(std::string name);
    Token token(Token::Type type, std::string lexeme, std::optional<DebugPosition> pos = {});
    char current_char();
    char advance();
...
public:
    Lexer() {}
    void setup(std::string code, std::string file);
